...
static GameType SyncCountdown(const Racedata& raceData) {
    ...
    const System* system = System::sInstance;
    if(system->GetBoolRadioContext(MODE_KO) && type == GAMETYPE_ONLINE_SPECTATOR) type = GAMETYPE_DEFAULT;
    return type;
}
...
static u8 SwapRaceMiis() { //this can only be called if localPlayerCount > 0, ie one of the players is still in, very hacky function but until a better hook is found, this'll do
    ...
    bool isKO = system->GetBoolRadioContext(MODE_KO);
    if(isKO) {
...
static bool CtrlRaceItemWindowIsInactive(const CtrlRaceItemWindow& itemWindow) {
    ...
    if(Racedata::sInstance->racesScenario.players[playerId].playerType == PLAYER_REAL_ONLINE && System::sInstance->GetBoolRadioContext(MODE_KO)) {
...
void StoreItemsForSpectating(RKNet::ITEMHandler& itemHandler) {
    ...
    if(System::sInstance->GetBoolRadioContext(MODE_KO)) { //guaranteed to be spectating already via a check in the func

...
void CondCollisions(Kart::Collision& collision, const Kart::Player& other) {
    if(System::sInstance->GetBoolRadioContext(MODE_OTT)) return;
...
void CondTTObjects(u32 r3, u32 r4, u32 r5, u32 r6, bool isTT) {
    ...
    asm(mr mgr, r31;);
    if(System::sInstance->GetBoolRadioContext(MODE_OTT)) isTT = true;
    mgr->isTT = isTT;
}
...
void CondStartPos(KMP::Holder<KTPT>& holder, Vec3& position, Vec3& rotation, u32 playerPosition, u32 playerCount) {
    if(System::sInstance->GetBoolRadioContext(MODE_OTT)) {
        playerPosition = 1;
        playerCount = 1;
    }
...
static bool CondPylonCollision(u32, u8 playerId) { //returns true if ghost collision, false if normal collision
    ...
    if(System::sInstance->GetBoolRadioContext(MODE_OTT) && type != PLAYER_REAL_LOCAL) type = PLAYER_GHOST;
...
static bool CondPoihanaStopOnHit(const Kart::Player& player) { //disables the poihana animating on hit if returns false
    ...
    if(System::sInstance->GetBoolRadioContext(MODE_OTT)) return !(bitfield & 0x2); //return false if player is local, as that is the only situation where the cataquack should stop
...
static void CondTTSeed(RacedataScenario& scenario) { //real non-ghost count is stored in system
    ...
    if(System::sInstance->GetBoolRadioContext(MODE_OTT)) scenario.settings.gamemode = MODE_TIME_TRIAL;
    scenario.InitRNG();
...
static void EnableOpacityFunctionality(EGG::ScnRenderer& renderer, u32 enabledEffectsFlag) {
    if(System::sInstance->GetBoolRadioContext(MODE_OTT)) enabledEffectsFlag |= 1;
...
static UnkType PreventBurnOuts(Kart::Status& status, s32 startBoostIdx) {
    if(startBoostIdx == -1 && System::sInstance->GetBoolRadioContext(MODE_OTT)) {
        startBoostIdx = 1;
...
static void PreventVoteChangeSection(Pages::Vote& vote, SectionId id, float delay) {
    ...
    if(system->GetBoolRadioContext(MODE_OTT)) {
        ...
        if(select->allowChangeComboStatus > 0) {
            ...
            if(system->GetBoolRadioContext(MODE_KO) && system->koMgr->isSpectating) {
...
static void MuteKartSounds(Audio::EngineMgr* mgr, Audio::KartActor* actor) {
    mgr->Init(actor);
    if(System::sInstance->GetBoolRadioContext(MODE_OTT) && !actor->isLocal && Settings::Mgr::Get().GetSettingValue(Settings::SETTINGSTYPE_OTTKO, SETTINGAUDIO_RADIO_OTTMUTEPTANDPLAYERS) == true) {
        actor->isGhost = true;
    }
}
...
static bool MuteCharSounds(Kart::Link* link) {
    ...
    if(System::sInstance->GetBoolRadioContext(MODE_OTT) && Settings::Mgr::Get().GetSettingValue(Settings::SETTINGSTYPE_OTTKO, SETTINGAUDIO_RADIO_OTTMUTEPTANDPLAYERS) == true) {
...
static bool MutePositionTracker(CtrlRaceRankNum& tracker) { //isInactive = muted if true, bctrl = IsInactive
    ...
    if(System::sInstance->GetBoolRadioContext(MODE_OTT) && !isInactive) isInactive = Settings::Mgr::Get().GetSettingValue(Settings::SETTINGSTYPE_OTTKO, SETTINGAUDIO_RADIO_OTTMUTEPTANDPLAYERS);
    return isInactive;
}

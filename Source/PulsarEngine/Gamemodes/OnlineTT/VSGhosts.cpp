...
void AddGhostToVS() {
    ...
    if(GameScene::GetCurrent()->id == SCENE_ID_RACE && system->GetBoolRadioContext(MODE_OTT) && racedata->menusScenario.settings.gametype != GAMETYPE_ONLINE_SPECTATOR) {
        u8 playerCount;
        u8 screenCount;
        u8 localCount;
        ...
        const TTMode ttMode = static_cast<TTMode>(racedata->menusScenario.settings.engineClass % 2 + 2 * ( system->GetContext(ITEM_START) == ITEMSETTING_START_FEAT ) ); //CC_150 (2) becomes 0 (TT_MODE_150), CC_100 (1) becomes 1 (TT_MODE_100)
        ...
        if(rkg->CheckValidity()) {
            if(rkg->header.kartId > HONEYCOUPE || rkg->header.driftType == 1 || system->GetFullRadioContext(PHYS_TURBO)) {

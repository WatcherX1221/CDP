...
void CorrectRoomStartButton(Pages::Globe::MessageWindow& control, u32 bmgId, Text::Info* info) {
    ...
    if (bmgId == BMG_PLAY_GP || bmgId == BMG_PLAY_TEAM_GP) {
        const u32 hostRadioContexts = System::sInstance->netMgr.hostRadioContexts;
        const bool isOTT = hostRadioContexts & (1 << MODE_OTT);
        const bool isKO = hostRadioContexts & (1 << MODE_KO);
        if (isOTT || isKO) {

...
static const u64 CreateSwitchFocusPlayerPtmfs(u32 arg) { //extremely hacky, but it does the job for now
    ...
    if(system->GetBoolRadioContext(MODE_KO) && system->koMgr->isSpectating || id >= SECTION_WATCH_GHOST_FROM_CHANNEL && id <= SECTION_WATCH_GHOST_FROM_MENU) id = SECTION_P1_WIFI_VS_LIVEVIEW;
    fakeSection = id;
...
static void RaceinfoNoSpectating() {
    ...
    asm(mr raceInfo, r28;);
    const System* system = System::sInstance;
    raceInfo->isSpectating = !system->GetBoolRadioContext(MODE_KO); //default instruction would store 1, here we only store 1 if it's not ko
}
kmCall(0x80532cf8, RaceinfoNoSpectating);

static bool SkipOpeningPanCheck(const RaceCameraMgr& cameraMgr) {
    const System* system = System::sInstance;
    if(system->GetBoolRadioContext(MODE_KO) && system->koMgr->isSpectating) return true;

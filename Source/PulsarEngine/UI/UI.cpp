...
//HAW In Order Page Circumvention
...
void ExpSection::CreatePages(ExpSection& self, SectionId id) {
    ...
    self.hasAutoVote = (id >= SECTION_P1_WIFI_FROOM_VS_VOTING && id <= SECTION_P2_WIFI_FROOM_COIN_VOTING) && system->GetFullRadioContext(HOST_HAW) > 0;
...
void ExpSection::CreatePulPages() {
    ...
    switch(this->sectionId) {
        ...
        case SECTION_P2_WIFI_FRIEND_TEAMVS:   //0x75
            ...
            if(system->GetFullRadioContext(HOST_HAW)) {
                if(system->GetFullRadioContext(HOST_HAW) < 2) {
                    ...
                    PulsarId next;
                    next = static_cast<PulsarId>(cupsConfig->GetSelected()+2); // +2 because 1 seems to do nothing
        ...
        case SECTION_P2_WIFI_FRIEND_COIN:     //0x77
            ...
            if(system->GetFullRadioContext(HOST_HAW) > 0) {

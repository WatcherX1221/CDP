...
void BeforeSELECTSend(RKNet::PacketHolder<PulSELECT>* packetHolder, PulSELECT* src, u32 len) { //len is sizeof(RKNet::SELECTPacket) by default
    ...
    if (!system->GetBoolRadioContext(PULSAR_CT)) {
...
void ExpSELECTHandler::DecideTrack(ExpSELECTHandler& self) {
    ...
    if (mode == RKNet::ONLINEMODE_PRIVATE_VS && system->GetBoolRadioContext(MODE_KO)) system->koMgr->PatchAids(sub);

    if (mode == RKNet::ONLINEMODE_PRIVATE_VS && Settings::Mgr::Get().GetSettingValue(Settings::SETTINGSTYPE_HOST, SETTINGHOST_RADIO_HOSTWINS) && !system->GetBoolRadioContext(MODE_KO)) {
        ...
        const bool isCT = system->GetBoolRadioContext(PULSAR_CT);
...
CourseId SetCorrectSlot(ExpSELECTHandler* select) {
    ...
    const System* system = System::sInstance;
    if (system->GetBoolRadioContext(MODE_KO) && system->koMgr->isSpectating) Racedata::sInstance->menusScenario.settings.gametype = GAMETYPE_ONLINE_SPECTATOR;
    return id;
}
...
static void DecideCC(ExpSELECTHandler& handler) {
    System* system = System::sInstance;
    if (!system->GetBoolRadioContext(PULSAR_CT)) reinterpret_cast<RKNet::SELECTHandler&>(handler).DecideEngineClass();
    else {

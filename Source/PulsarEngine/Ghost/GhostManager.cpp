...
bool Mgr::SaveGhost(const RKSYS::LicenseLdbEntry& entry, u32 ldbPosition, bool isFlap) {
    ...
    const bool validTTs = (mode != TTMODE_UNRESTRICTED) && System::sInstance->GetBoolRadioContext(TTS_VALID);
    ...
    if(data.CreateRKG(buffer) && buffer.CompressTo(this->rkg)) {
        ...
        if(ldbPosition >= 0 && validTTs) this->leaderboard.Update(ldbPosition, entry, crc32); //in this order as save opens files too
        system->taskThread->Request(&Mgr::CreateAndSaveFiles, this, 0);

        const Timer& expert = this->GetExpert();
        if(expert.isActive && expert > entry.timer && system->GetInfo().HasTrophies() && validTTs ) {
            gotTrophy = true;
...
static bool RacedataCheckCorrectRKG() {
    u8 offset = 0;
    u32 index;
    if(System::sInstance->GetBoolRadioContext(MODE_OTT)) index = 0;
    else {
        register u32 id;
        asm(mr id, r27;);

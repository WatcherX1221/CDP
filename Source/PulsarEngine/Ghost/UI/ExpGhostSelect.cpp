...
void BeforeEntranceAnimations(Pages::TTSplits* page) {
    ...
    //Finish Time Leaderboard check and request
        ...
        if (position == 0) {
            ...
            page->ctrlRaceTimeArray[0]->EnableFlashingAnimation();
            page->PlaySound(SOUND_ID_NEW_RECORD, -1);
            if(System::sInstance->GetBoolRadioContext(TTS_VALID)) {page->savedGhostMessage.SetMessage(UI::BMG_SAVED_GHOST);} else page->savedGhostMessage.SetMessage(UI::BMG_TROPHY_INVALID);
        ...
        if (hasFlap || position >= 0) {
            ...
            if (gotTrophy) {
                if(System::sInstance->GetBoolRadioContext(TTS_VALID)) {page->savedGhostMessage.SetMessage(UI::BMG_TROPHY_EARNED);} else page->savedGhostMessage.SetMessage(UI::BMG_TROPHY_INVALID);
            }
        }
    }
}

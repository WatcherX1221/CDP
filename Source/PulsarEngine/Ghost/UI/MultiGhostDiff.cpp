...
u32 MultiGhostDiff::Count() {
    return System::sInstance->GetBoolRadioContext(MODE_OTT);
}
...
MultiGhostDiff::MultiGhostDiff() {
    u32 count = 0;
    if(System::sInstance->GetBoolRadioContext(MODE_OTT)) count = 2;
    else {
...
void MultiGhostDiff::Load() {
    if(this->diffTimeCount > 0) {
        if(System::sInstance->GetBoolRadioContext(MODE_OTT)) this->diffTimes = new OTTGhostDiff[this->diffTimeCount];
        ...
        for(int i = 0; i < this->diffTimeCount; ++i) {
            ...
            if(!System::sInstance->GetBoolRadioContext(MODE_OTT)) this->diffTimes[i].ghostData.Init(Racedata::sInstance->ghosts[i + this->isGhostReplay]);
